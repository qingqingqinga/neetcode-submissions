class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        def dfs(index,row,col):
            ...
            if row < 0 or row >=rows or col < 0 or col >= cols or word[index] != board[row][col] or visited[row][col]:
                return False
            
            visited[row][col] = True

            if (dfs(index + 1,row - 1,col )or
        ...
        visisted = [[False] * cols for _ in range(rows)]

        def dfs(row,col,index):
            if index >= len(word):
                return True
            
            if (row >= rows or row < 0 or col >= cols or col < 0 or word[index] != board[row][col] or visited[row][col]):
                return False
